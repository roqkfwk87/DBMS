...
#! 내부 조인의 기본 형태 -- 그냥 join을 불러도 내부 조인이 불러짐 
# select 열 목록
# from 첫 번째 테이블 -- 기준이 되는 테이블을 가져옴
# 		(inner) joun 두 번째 테이블 -- inner(내부) join에서 inner 생략 가능
# 		on 조인될 조건 
# [where 조건...] 

#! 내부 조인 예제
# 구매 테이블에서 '1' member_id를 가진 사람이 구매한 
# 구매한 물건을 발송하기 위해 조인 
...
# 조인 시 두 개의 이상의 테이블에서 동일한 열 이름이 존재할 경우
# 테이블명.열이름 형식으로 표기! 권장 

# 금액이 특정 값 이상인 모든 구매 내역을 조회 
select 
	M.name, purchase_date, B.amount 
from
	member M -- 해당 테이블의 첫 글자를 대문자로 표시한다., as 키워드 생략이 가능하기 때문
inner join buy B
